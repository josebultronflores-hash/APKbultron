...
class InformeQlsScreen extends StatefulWidget {
  final DocumentoViatico? documentoEditar;

  const InformeQlsScreen({
    super.key,
    this.documentoEditar,
  });

  @override
  State<InformeQlsScreen> createState() => _InformeQlsScreenState();
}

class _InformeQlsScreenState extends State<InformeQlsScreen> {
...
bool get requiereTasaCambio {
  ...
  void initState() {
    ...
    idDocumentoActual = widget.documentoEditar?.id ??
        'informe_qls_${DateTime.now().millisecondsSinceEpoch}';
    ...
    if (widget.documentoEditar != null) {
      _cargarDatosInformeQls(widget.documentoEditar!.datosFormulario);
    }
...
double _valorDiaValido(FilaGasto row, int dayIndex) {
  ...
  // El valor de la celda ya se convierte a USD cuando el usuario sale del campo.
  // Aquí no se vuelve a convertir para evitar doble conversión.
...
DateTime _inicioSemanaDomingo(DateTime fecha) {
  ...
  // Dart: lunes = 1, martes = 2, ..., domingo = 7
  // Formulario: domingo = 0, lunes = 1, ..., sábado = 6
...
void _eliminarMiscelaneo(int index) {
  // Protege las filas base que existen en la plantilla PDF:
  // Propina, Agua, Herramientas y Atención al cliente.
  if (index < 4) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
...
Map<String, dynamic> _obtenerDatosInformeQls() {
  return {
    ...
    'fechasDias': fechaDiaControllers
        .map((controller) => controller.text)
        .toList(),

    'lugaresDias': lugarDiaControllers
        .map((controller) => controller.text)
        .toList(),
...
void _cargarDatosInformeQls(Map<String, dynamic> datos) {
  ...
  codigoMonedaSeleccionada =
      (datos['codigoMoneda'] ?? 'USD').toString();
  ...
  otrosPagadosController.text =
      (datos['otrosPagados'] ?? '0.00').toString();
  ...
  fechaRevisadoController.text =
      (datos['fechaRevisado'] ?? '').toString();
  ...
  if (fechasDias is List) {
    for (int i = 0; i < fechaDiaControllers.length && i < fechasDias.length; i++) {
  ...
  if (lugaresDias is List) {
    for (int i = 0; i < lugarDiaControllers.length && i < lugaresDias.length; i++) {
...
void _cargarFilasGasto(
  ...
  for (int i = 0; i < filas.length && i < filasGuardadas.length; i++) {
    ...
    if (valores is List) {
      for (int j = 0; j < filas[i].controllers.length && j < valores.length; j++) {
    ...
    if (convertido is List) {
      for (int j = 0; j < filas[i].convertidoADolares.length && j < convertido.length; j++) {
...
void _cargarMiscelaneos(dynamic filasGuardadas) {
  ...
  for (final filaGuardada in filasGuardadas) {
    ...
    final convertido = filaGuardada['convertidoADolares'];
    if (convertido is List) {
      for (int j = 0; j < fila.convertidoADolares.length && j < convertido.length; j++) {
...
bool _validarDatosMinimos() {
  ...
  if (preparadoPor.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Ingrese el nombre del solicitante'),
        backgroundColor: Colors.orange,
      ),
    );
    return false;
...
Future<void> _guardarInformeQls({
  String pdfPath = '',
  ...
  final documento = DocumentoViatico(
    ...
    datosFormulario: _obtenerDatosInformeQls(),
...
Future<Uint8List> _crearPdfInformeQls() async {
  final ByteData plantillaData = await rootBundle.load(
    'assets/templates/informe_qls_template.png',
  );
  ...
  // Coordenadas horizontales para montos.
  // Están alineadas a la derecha dentro de cada columna.
  ...
  // Coordenadas para fecha/localización.
  // Estas van centradas dentro de cada celda.
  ...
  // Si agregas más misceláneos manualmente, sus montos se suman en el total,
  // pero solo las primeras 4 filas se imprimen en la plantilla porque son las que existen visualmente.
  agregarTotalesPorDia(
    widgets: widgets,
  ...
  // =========================
  // 6. Total de gastos por justificar
  // =========================
  agregarTotalesPorDia(
    widgets: widgets,
    filas: [
...
for (int i = 0; i < detalles.length && i < 8; i++) {
  ...
  // Ajustado para que cada detalle caiga dentro de su línea
  final top = 441 + (i * 8.5);
...
widgets.addAll([
  textoPdf(
    preparadoPorController.text.isEmpty
        ? empleado
        : preparadoPorController.text,
    66,
    710,
...
Future<void> _generarPdfPendiente() async {
  ...
  try {
    final Uint8List pdfBytes = await _crearPdfInformeQls();

    final Directory directory = await getApplicationDocumentsDirectory();

    final String nombreArchivo =
        'informe_qls_${DateTime.now().millisecondsSinceEpoch}.pdf';

    final File archivoPdf = File('${directory.path}/$nombreArchivo');

    await archivoPdf.writeAsBytes(pdfBytes);

    await _guardarInformeQls(
  ...
  return TextFormField(
    ...
    decoration: InputDecoration(
      labelText: requiereTasaCambio
          ...
          : 'Tasa de cambio',
      hintText: requiereTasaCambio
          ? 'Ejemplo: 17.80'
          : 'No requerida para USD',
...
String _textoConceptoCompacto(String texto) {
  ...
  // Saltos visuales para mantener la columna Concepto más angosta.
  // No cambia los nombres usados para cálculos ni para el PDF; solo cambia cómo se ven en pantalla.
...
double _calcularAnchoColumnaConcepto(
  ...
  // Ancho fijo para que todas las columnas Concepto tengan el mismo tamaño.
  // El texto largo se parte en varias líneas con _textoConceptoCompacto().
  return 106.0;
}
 ...
 Widget _detalleGastos() {
  ...
  return Column(
    children: [
      ...
      // 1. Adelanto al colaborador - editable
      ...
      // 3. Reembolso por el colaborador - automático
      _filaResumenAutomatico(
      ...
      // 4. Monto por pagar al colaborador - automático
      _filaResumenAutomatico(
...
Widget _botonesFinales() {
        ...
        child: OutlinedButton.icon(
         onPressed: () async {
            ...
            await mostrarCheckGuardado(
              context,
              mensaje: 'Informe QLS guardado correctamente',
            );
            },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Informe QLS Panamá'),
        centerTitle: true,
